...
serve(async (req) => {
  ...
  const { data: order, error: orderErr } = await admin
    .from("orders")
    .select(
      "id, email, subtotal, shipping, total, stripe_session_id, correos_shipment_code, order_items ( product_name, quantity, unit_price )",
    )
    .eq("id", orderId)
    .maybeSingle();

  if (orderErr || !order) {
    console.error("send_customer_order_tracking_fetch", orderErr);
  ...
  if (!code) {
    return new Response(JSON.stringify({ error: "Order has no Correos tracking code" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (!email) {
    return new Response(JSON.stringify({ error: "Order has no email" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (!lines.length) {
    return new Response(JSON.stringify({ error: "Order has no lines" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (!response.ok) {
    ...
    console.error("send_customer_order_tracking_resend", errText);
