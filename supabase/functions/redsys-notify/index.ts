...
function decodeMerchantDataB64(b64: string): { email: string; lines: Array<{ productId: string; quantity: number }>; shippingEur: number; subtotalEur: number } | null {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    const merchantDataB64 = pick(decoded, [
      "Ds_MerchantData",
      "DS_MERCHANTDATA",
    ...
    const { data: existing } = await admin.from("orders").select("id").eq("stripe_session_id", dsOrder).maybeSingle();
    if (existing?.id) {
      return new Response("OK", { status: 200, headers: { "Content-Type": "text/plain" } });
    }
    ...
    const orderItems = payload.lines.map((line) => {
