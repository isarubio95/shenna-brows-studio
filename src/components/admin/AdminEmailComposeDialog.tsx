...
type AdminEmailComposeDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  title: string;
  recipientHint?: string;
  sendButtonLabel?: string;
  onSend: (payload: AdminEmailSendPayload) => Promise<{ ok: boolean; message?: string; error?: string; status?: number }>;
};

const AdminEmailComposeDialog = ({
  open,
  onOpenChange,
  title,
  recipientHint,
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      if (result.ok) {
        toast({ title: result.message ? `✅ ${result.message}` : "✅ Email enviado" });
        handleOpenChange(false);
      } else {
        if (result.status === 429) setCooldownUntil(Date.now() + 30000);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-playfair text-carbon">{title}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {recipientHint ? (
            <div>
              <label className="text-xs text-carbon/50 block mb-1">Para</label>
              <div className="text-sm text-carbon bg-muted/30 px-3 py-2 rounded-md">{recipientHint}</div>
            </div>
          ) : null}
              ...
              <div className="flex flex-wrap gap-2">
                {attachments.map((att, i) => (
                  <div key={i} className="flex items-center gap-1.5 bg-muted/30 rounded-md px-2.5 py-1.5 text-xs text-carbon/70">
                    ...
                    <button type="button" onClick={() => removeAttachment(i)} className="text-carbon/40 hover:text-red-500 transition-colors">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
        ...
        <DialogFooter>
          ...
          <Button onClick={sendEmail} disabled={sending || !subject.trim()} className="bg-gold hover:bg-gold/90 text-white">
            {sending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Send className="h-4 w-4 mr-2" />}
            {sending ? "Enviando..." : sendButtonLabel}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default AdminEmailComposeDialog;
